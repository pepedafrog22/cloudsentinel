...
def build_graph(findings: List[Dict[str, Any]]) -> Tuple[Network, Dict[str, Any]]:
    ...
    _add_node(net, G, "hdr::principals", label="Principals", title="", color=CLR_SUMMARY, size=14, level=0, shape="box", fixed=True, x=-900, y=-100)
_add_node(net, G, "hdr::roles",      label="Roles",      title="", color=CLR_SUMMARY, size=14, level=1, shape="box", fixed=True, x=-300, y=-100)
_add_node(net, G, "hdr::services",   label="Services",   title="", color=CLR_SUMMARY, size=14, level=2, shape="box", fixed=True, x= 300, y=-100)
