...
def analyze_escalation_ready(findings: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    ...
    Returns a dict keyed by role name with the above info.
    """
    ...
    for f in findings:
        ...
        r = roles.setdefault(role, {
            ...
            "exploitable_actions": set(),  # set of "svc:Action"
        ...
        if rule == "R2":
            # attempt to pull actions from statement
            ...
            for a in acts:
                ...
                if not parsed:
                    continue
                svc, act = parsed
                # svc lowercased; compare against exploit map
    ...
    for role, info in roles.items():
        if info["has_passrole_star"] and info["exploitable_actions"]:
            ...
            # pick one exploitable action to create a concise exploit string
            # e.g., "RunInstances(IamInstanceProfile=ANY_ROLE)"
            svc_act = sorted(info["exploitable_actions"])[0]
            svc, act = svc_act.split(":", 1)
            # map common action to a readable exploit description
