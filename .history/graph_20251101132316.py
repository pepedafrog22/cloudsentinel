...
def build_graph_from_findings(findings: List[Dict[str, Any]]) -> Tuple[nx.DiGraph, Network]:
    ...
    # Use a hierarchical left→right layout, turn off physics (stable)
    net.set_options("""
    var options = {
      layout: { hierarchical: { enabled: true, levelSeparation: 220, nodeSpacing: 220, direction: 'LR', sortMethod: 'directed' } },
    ...
    for f in findings:
        ...
        # R2: role -> service nodes (e.g., s3:*)
            ...
            if not services:
                # fallback to generic service target
    ...
    # Add a simple legend box (fixed in corner)
    net.add_node(
        "legend",
        label="Legend\nR1 PassRole on *  (red)\nR2 svc:* on *     (orange)\nR3 Trusts          (purple)",
        shape="box",
