...
class CloudSentinelCollector:
    ...
    def _collect_users(self) -> None:
        ...
        self.snapshot.setdefault("iam", {})["users"] = users
    ...
    def _collect_roles(self) -> None:
        ...
        self.snapshot.setdefault("iam", {})["roles"] = roles
            ...
            try:
                ...
                if p:
                    ...
                    try:
                        ...
                        print(f"[!] list_attached_role_policies({role_name}) failed: {e}")

                # Fetch each attached policy's default document
                for ap in role["AttachedPolicies"]:
                    ...
                    try:
                        ...
                        if not version_id:
                            continue
                        policy_ver = self.iam.get_policy_version(
                            PolicyArn=policy_arn, VersionId=version_id
                        )
                        ap["PolicyDocument"] = policy_ver.get("PolicyVersion", {}).get("Document", {})
                    except ClientError as e:
                        print(f"[!] get_policy/version failed {policy_arn} for {role_name}: {e}")
...
if __name__ == "__main__":
    ...
    if args.demo:
        ...
        data = read_json(args.demo)
        data.setdefault("collected_at", iso_utc_now())
        data.setdefault("iam", {})
        write_json(data, args.out)
        print(f"[+] Wrote demo snapshot to {args.out}")
    else:
        print("[*] Starting live IAM collection (read-only).")
        collector = CloudSentinelCollector(profile=args.profile)
        # Debug: ensure collect() exists
        # print([k for k in CloudSentinelCollector.__dict__.keys() if "collect" in k])
        print("Running:", __file__)
        print("Class attrs:", repr([k for k in CloudSentinelCollector.__dict__.keys()]))
