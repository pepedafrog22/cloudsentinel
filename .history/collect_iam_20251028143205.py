...
Before we continue, i have a question. So in the for loop we looped over the roles list which contains a dictionary that represents every role. Now in every iteration we get back a dictionary from the roles list, and then we get the value of the "RoleName" key and store it in a variable. We use that to make our get_role API call, and the response is a dictionary that contains the role key and the value of that role key is a dictionary that contains the AssumeRolePolicyDocument key, and that itself is a dictionary. Now my question is, why are we doing: role["AssumeRolePolicyDocument"] = trust? aren't we just storing the contents of the AssumeRolePolicyDocument inside the dictionary that's inside the roles list, and not inside the self.snapshot dictionary?
