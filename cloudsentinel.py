...
python cloudsentinel.py cloudsentinel --in-snapshot <path to canned snapshot json file>

# Start from existing findings (read demo/findings.json) → just render the ASCII report
python cloudsentinel.py cloudsentinel --in-findings demo/findings.json --report-out out/report.txt

# Also expose pass-through wrappers so you can call the modules that make up cloudsentinel seperately for debugging purposes via this CLI:
python cloudsentinel.py collect --profile <profile name> --out data/iam_snapshot.json
python cloudsentinel.py analyze --in demo/sample_account.json --out out/findings.json --format json
python cloudsentinel.py report --in demo/findings.json --out out/report.txt
"""
